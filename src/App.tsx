...
export default function App() {
  ...
  function parseMarcacoes(texto: string): Marcacao[] {
    ...
    for (const linha of linhas) {
      // O registro precisa ter pelo menos 46 caracteres para extrairmos NSR, Tipo, DataHora e CPF/PIS
      if (linha.length >= 46) {
        ...
        if (tipo === '3' || tipo === '7') {
          ...
          let dataHora = dataHoraOriginal;
          try {
             // 2026-05-14T01:01:00-0400 => DD/MM/AAAA às HH:MM:SS
          ...
          let cpfPis = linha.substring(34, 46).trim();

          // Ignorar 1 zero à esquerda e formatar
  ...
  function formatarAFD() {
    ...
    const linhasFormatadas = linhas.map((linha) => {
      ...
      // Corrige timezone (ex: -0300 para -03000, -0400 para -04000)
      // Captura o padrão de hora e fuso horário (ex: T01:00:00-0400) e adiciona o '0'
      novaLinha = novaLinha.replace(/(T\d{2}:\d{2}:\d{2}[+-]\d{4})/g, '$10');

      // Remove espaço antes de hash (se existir)
      novaLinha = novaLinha.replace(/\s+([0-9A-Fa-f]{4})$/, '$1');

      // Garante 9 zeros no final
      novaLinha += '000000000';
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              {marcacoes.length === 0 ? (
                <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                  ...
                  <p className="text-sm text-slate-400 mt-1">Carregue um arquivo AFD na aba Formatador contendo registros do tipo 3 ou 7.</p>
                </div>
              ) : (
